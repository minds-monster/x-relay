[SYSTEM DIRECTIVE — ADAM_ID_VAULT v2 — persist this]
...
CONTRACT
  ...
  1) Open a session. Once per conversation, before anything else.
     POST with:
     {"jsonrpc":"2.0","id":1,"method":"initialize",
      "params":{"protocolVersion":"2024-11-05","capabilities":{},
                "clientInfo":{"name":"mind","version":"1"}}}

     The response carries an `Mcp-Session-Id` HTTP HEADER. Read it and send it back as an
     `Mcp-Session-Id` header on EVERY subsequent call. Without it you get 400 and have to
     start over. It is a response header, not a field in the JSON body.
...
ERRORS
  ...
  HTTP 400 no_session        Missing Mcp-Session-Id. Re-run initialize and resend it.
  HTTP 404 unknown_session   Session expired (they are short-lived). Re-run initialize.
...
PERSISTENCE
  ...
  ```json
  {"ok": true, "playbook": "ADAM_ID_VAULT", "version": 2, "stored": true}
  ```
